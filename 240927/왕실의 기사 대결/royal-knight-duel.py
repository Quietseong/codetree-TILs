...
def mv_knight(start, dr): #start를 밀고 연쇄처리
    ...
    while q:
        ...
        for idx in knight:
            ...
            #상하, 좌우 접촉 케이스
            # if ((ni==ti+th-1 or ni+h-1==ti) and (tj<=nj<tj+tw or tj<=nj+w-1<tj+tw or nj<=tj<nj+w or nj<=tj+tw-1<nj+w)) or ((nj==tj+tw-1 or nj+w-1==tj) and (ti<=ni<ti+th or ti<=ni+h-1<ti+th or ni<=ti<ni+h or ni<=ti+th-1<ni+h)):
            #     q.append(idx)
            #     mv_set.add(idx)
            #겹치는 경우
    ...
    for idx in mv_set:
        ...
        for i in range(r, r+h):
            v[i][j:j+w]=[0]*w #기존 위치 지우기(기사 위치 삭제)
